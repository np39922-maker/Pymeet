...
## Docker Desktop (recommended local setup)

```powershell
copy .env.example .env
# Edit .env and replace both placeholder secrets first.
...
The backend and PostgreSQL are intentionally private Docker services. Nginx serves the
React build and proxies `/api` and `/socket.io`, so the same image works on localhost
and on a live HTTPS domain without hard-coded browser URLs.
...
For Windows Server 2022, WSL, DNS, HTTPS, firewall, backups, and TURN guidance, see
